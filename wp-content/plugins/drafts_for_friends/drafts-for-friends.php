...
class DraftsForFriends	{
	...
	function process_extend( $params ) {
		...
		if ( $valid ) {
			...
			$this->user_shares = $shared;
			$this->save_shares();
			$result = esc_html__( 'Share extended.', 'drafts_for_friends' );
		} else {
			$result = esc_html__( 'Failed extending share.', 'drafts_for_friends' );
		}
		return $result;
	}
						...
						if( is_array( $shared_posts ) || $shared_posts instanceof \Traversable ):
							foreach ( $shared_posts as $share ):
								...
								<tr class="<?php echo $this->get_expiration_class($share['expires']); ?>">
									...
									<td><input type="text" size="60" value="<?php echo esc_attr( $url ); ?>" /></td>
									<td class="actions">
